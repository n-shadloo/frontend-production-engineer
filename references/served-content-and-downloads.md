...
The backend's security review owns the server-side check over the stored file,
and the response headers that Django sends.
...
- The server-side check over a stored file, the name that it is stored under,
  and the headers that Django sends → the backend's security review.
