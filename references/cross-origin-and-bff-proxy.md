...
The backend's security review owns the server settings and their risk. NEVER work around
...
network tab before you change any frontend code. The backend's security review
owns the server-side choice of each attribute.
...
- The DRF permission class, the server-side CSRF enforcement, the CORS settings,
  and the cookie attributes → the backend's security review. This file owns what the
