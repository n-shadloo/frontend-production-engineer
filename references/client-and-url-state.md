...
| The user expects a link, a bookmark, or the back button to restore it. It is a filter, a sort, a page, a tab, a search term, or an open record | The URL, as a search param | The value is a secret, or it is too large for an address bar. | Every change writes a history entry, and every read passes through a parser. |
