...
description: >-
  ...
  reviewed and the task touches App Router files (app/, proxy.ts), the
  "use client" or "use server" boundary, await params, a Server
  Action, a React hook, a cast (any), a Zod schema, the DRF contract
  (an OpenAPI schema, CORS, CSRF), the client cache (TanStack Query,
  useQuery), live data (a WebSocket), auth (a session, an httpOnly
  cookie, a token refresh, a protected route), styling (Tailwind, a
  design token, dark mode, shadcn), accessibility (WCAG, ARIA,
  aria-label, a keyboard, a screen reader, axe), forms (a form,
  useForm, a field error, unsaved changes), data surfaces (a data
  table, pagination, a chart, an export), media (an upload,
  next/image, a video, a download), or the project setup
  (package.json, tsconfig.json, eslint.config.ts), and the agent has
...
Forty-two seams cross the domains, and this table settles each one.
...
The router table above is the integrated material at 1.12.0, and it is the
