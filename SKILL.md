...
description: >-
  ...
  reviewed and the task touches App Router files (app/, page.tsx,
  proxy.ts), the "use client" or "use server" boundary, await params,
  a Server Action, a React hook, a cast (any), a Zod schema, the DRF
  contract (an OpenAPI schema, a generated client, CORS, CSRF), the
  client cache (TanStack Query, useQuery), live data (a WebSocket, a
  pushed event), auth (a session, an httpOnly cookie, a token refresh,
  a protected route), styling (Tailwind, a design token, dark mode,
  shadcn), accessibility (WCAG, ARIA, aria-label, a keyboard, a screen
  reader, axe), forms (a form, useForm, a field error, unsaved
  changes), data surfaces (a data table, a sort, pagination, a chart,
  an export), or the project setup (package.json, tsconfig.json,
...
Thirty-five seams cross the domains, and this table settles each one.
