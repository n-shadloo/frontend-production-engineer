...
description: >-
  ...
  cast, Zod), DRF contract (OpenAPI, CORS, CSRF), cache (TanStack
  Query, useQuery), live data (WebSocket), auth (session, token),
  security (XSS, CSP, CVE), styling (Tailwind, shadcn), accessibility
  (WCAG, ARIA, keyboard, screen reader, axe), forms (useForm), data
  surfaces (table, chart), media (upload, next/image), motion
  (animation), copy (label, error message, empty state), performance
  (slow, LCP, INP, CLS, bundle size), SEO (metadata, sitemap,
  robots.txt), i18n (locale, RTL, translation), tests (Vitest,
  Playwright, MSW, flaky, coverage), observability (Sentry, offline,
  health check), deploy (Docker, Nginx, CI), or setup (package.json,
  tsconfig.json), and the agent must verify the installed versions
...
| What enters the repository and how a change leaves it — pnpm, npm, yarn, bun, `pnpm install --frozen-lockfile`, `pnpm-lock.yaml`, a lockfile conflict, a hand-edited lockfile, `packageManager`, Corepack, `corepack enable`, Volta, `.nvmrc`, `engines.node`, `.npmrc`, `overrides`; `minimumReleaseAge`, `minimumReleaseAgeExclude`, a cooldown, `onlyBuiltDependencies`, `allowBuilds`, `ignore-scripts`, a lifecycle script, `pnpm audit`, a new dependency and its justification; Renovate, `renovate.json`, Dependabot, `.github/dependabot.yml`, `automerge`, `matchUpdateTypes`, `fetch-metadata`; lefthook, `lefthook.yml`, Husky, `.husky/`, `lint-staged`, `stage_fixed`, a `prepare` script, a hook that never fires; commitlint, `commitlint.config.js`, a Conventional Commit, a commit-msg hook; `.gitignore`, a committed `.env.local`, gitleaks, `.gitleaks.toml`, a secret in the history; `CODEOWNERS`, `.gitattributes`, `linguist-generated`. Not here: the body of a script that a hook calls (`references/lint-format-and-scripts.md`), the folder that a new file goes into (`references/directory-and-module-boundaries.md`), the `NEXT_PUBLIC_` prefix (`references/app-router-structure.md`), whether a package is dangerous (`references/secret-boundary-and-supply-chain.md`), the CI workflow and the deploy (`references/release-pipeline-and-rollback.md`), the server-side secret storage (sibling skill `secure-code-auditor`). | `references/dependencies-and-git-workflow.md` |
...
One hundred and thirty-nine seams cross the domains, and this table settles each
one.
...
The router table above is the integrated material at 1.21.0, and it is the
