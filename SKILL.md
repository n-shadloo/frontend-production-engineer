...
description: >-
  ...
  reviewed and it touches App Router files (app/, proxy.ts), the "use
  client" or "use server" boundary, await params, a Server Action, a
  React hook, a cast (any), a Zod schema, the DRF contract (OpenAPI,
  CORS, CSRF), the client cache (TanStack Query, useQuery), live data
  (a WebSocket), auth (a session, a cookie, a token refresh), styling
  (Tailwind, a design token, dark mode, shadcn), accessibility (WCAG,
  ARIA, a keyboard, a screen reader, axe), forms (useForm, a field
  error, unsaved changes), data surfaces (a data table, a chart, an
  export), media (an upload, next/image, a video), motion (an
  animation, a transition, a drag), copy (a label, an error message,
  an empty state), or the project setup (package.json, tsconfig.json,
  eslint.config.ts), and the agent must verify the installed versions,
  plan first, and hold it to a definition of done, even if "frontend"
  is never used.
license: MIT
...
Sixty seams cross the domains, and this table settles each one.
...
The router table above is the integrated material at 1.14.0, and it is the
