...
description: >-
  ...
  WebSocket), auth (a session, a token), styling (Tailwind, a token,
  dark mode, shadcn), accessibility (WCAG, ARIA, a keyboard, a screen
  reader, axe), forms (useForm, a field error), data surfaces (a
  table, a chart, an export), media (an upload, next/image), motion
  (an animation, a transition), copy (a label, an error message, an
  empty state), performance (slow, LCP, INP, CLS, bundle size, a
  third-party script, Lighthouse), or the setup (package.json,
  tsconfig.json, eslint.config.ts), and the agent must verify the
  installed versions and plan first, even if "frontend" is never
  used.
license: MIT
...
Seventy-one seams cross the domains, and this table settles each one.
...
The router table above is the integrated material at 1.15.0, and it is the
