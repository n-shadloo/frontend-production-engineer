...
description: >-
  ...
  Server Action, a hydration error, a React hook (useState, useEffect,
  useActionState), a <Suspense> or error boundary, a type or a cast (any,
  unknown, a discriminated union), a Zod schema (safeParse), the DRF
  contract (an OpenAPI schema, drf-spectacular, a generated client, a
  pagination envelope, CORS, CSRF), the client cache and state (TanStack
  Query, useQuery, useMutation, staleTime, an optimistic update, a search
  param, nuqs, Zustand), or the project setup (package.json,
  eslint.config.ts, the lockfile, a git hook), and the agent has to verify
  the installed versions, plan first, and hold the work to a definition of
...
Eight seams cross the domains, and this table settles each one.
