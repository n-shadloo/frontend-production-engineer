...
description: >-
  ...
  dangerouslySetInnerHTML, SSRF, a CVE), styling (Tailwind, shadcn),
  accessibility (WCAG, ARIA, a keyboard, a screen reader, axe), forms
  (useForm, a field error), data surfaces (a table, a chart), media
  (an upload, next/image), motion (an animation), copy (a label, an
  error message, an empty state), performance (slow, LCP, INP, CLS,
  bundle size), SEO (metadata, a sitemap, robots.txt, a canonical
  URL), or the setup (package.json, tsconfig.json), and the agent must
  verify the installed versions first, even if "frontend" is never
  used.
license: MIT
...
Ninety-three seams cross the domains, and this table settles each one.
