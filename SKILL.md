...
description: >-
  ...
  cast, Zod), the DRF contract (OpenAPI, CORS, CSRF), the cache
  (TanStack Query, useQuery), live data (WebSocket), auth (session,
  token), security (XSS, CSP, nonce, CVE), styling (Tailwind, shadcn),
  accessibility (WCAG, ARIA, keyboard, screen reader, axe), forms
  (useForm, field error), data surfaces (table, chart), media (upload,
  next/image), motion (animation), copy (label, error message, empty
  state), performance (slow, LCP, INP, CLS, bundle size), SEO
  (metadata, sitemap, robots.txt, canonical URL), i18n (locale, RTL,
  translation, Intl), tests (Vitest, Playwright, MSW, flaky,
  coverage), or the setup (package.json, tsconfig.json), and the agent
  must verify the installed versions first, even if "frontend" is
  never used.
license: MIT
...
One hundred and eighteen seams cross the domains, and this table settles each
one.
...
The router table above is the integrated material at 1.19.0, and it is the
