...
description: >-
  ...
  touches App Router files (app/, proxy.ts), the "use client" or "use
  server" boundary, await params, a Server Action, a React hook, a
  cast (any), a Zod schema, the DRF contract (OpenAPI, CORS, CSRF),
  the client cache (TanStack Query, useQuery), live data (a
  WebSocket), auth (a session, a token), security (XSS, CSP, a nonce,
  a CVE), styling (Tailwind, shadcn), accessibility (WCAG, ARIA, a
  keyboard, a screen reader, axe), forms (useForm, a field error),
  data surfaces (a table, a chart), media (an upload, next/image),
  motion (an animation), copy (a label, an error message, an empty
  state), performance (slow, LCP, INP, CLS, bundle size), SEO
  (metadata, a sitemap, robots.txt, a canonical URL), i18n (a locale,
  RTL, a translation, Intl), or the setup (package.json,
...
One hundred and four seams cross the domains, and this table settles each one.
...
The router table above is the integrated material at 1.18.0, and it is the
