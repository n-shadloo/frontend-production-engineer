...
description: >-
  ...
  security (XSS, CSP, nonce, CVE), styling (Tailwind, shadcn),
  accessibility (WCAG, ARIA, keyboard, screen reader, axe), forms
  (useForm), data surfaces (table, chart), media (upload, next/image),
  motion (animation), copy (label, error message, empty state),
  performance (slow, LCP, INP, CLS, bundle size), SEO (metadata,
  sitemap, robots.txt), i18n (locale, RTL, translation, Intl), tests
  (Vitest, Playwright, MSW, flaky, coverage), observability (Sentry,
  offline, health check), or setup (package.json, tsconfig.json), and
...
One hundred and twenty-seven seams cross the domains, and this table settles each
one.
